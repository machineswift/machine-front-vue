...
<script setup lang="ts">
  ...
  import type { IamOrganizationExpandTreeResponseVo } from '@/modules/iam/types'

  const props = defineProps({
    organizationTree: {
      type: Object as PropType<IamOrganizationExpandTreeResponseVo | null>,
      required: false,
      default: () => ({ children: [] })
    }
  })
  ...
  const state = reactive({
    ...
    visible: false,
    organizationQuery: '',
    organizationTreeOptions: [] as IamOrganizationExpandTreeResponseVo[],
    organizationProps: {
      value: 'id',
      label: 'name',
      children: 'children'
    ...
    currentSelectedNode: null as IamOrganizationExpandTreeResponseVo | null,
    // 新增状态，用于存储默认展开的keys
   ...
   * 获取前两层节点的ID
   */
  const getFirstTwoLevelNodeIds = (nodes: IamOrganizationExpandTreeResponseVo[], level = 1, result: string[] = []): string[] => {
   ...
   * 组织树节点过滤方法
   */
  const organizationFilterMethod = (query: string, node: IamOrganizationExpandTreeResponseVo) => {
   ...
   * 处理树节点点击事件
   */
  const handleNodeClick = (node: IamOrganizationExpandTreeResponseVo) => {
    // 不能选择自己作为父节点
  ...
  const handleSubmit = async () => {
    ...
    try {
      state.loading = true
      await IamOrganizationApi.updateParent({
        id: state.form.id,
