<template>
  <div ref="pageContainerRef" class="organization-page">
    ...
    <!-- 对话框组件 -->
    <IamOrganizationCreateDialog v-model="currentTabState.dialogVisible.create" :parent-node="currentTabState.currentNode" @success="handleSuccess" />
    <IamOrganizationEditDialog v-model="currentTabState.dialogVisible.edit" :organization-id="currentTabState.currentOrganizationId" @success="handleSuccess" />
    <IamOrganizationUpdateParentDialog ref="updateParentDialog" :organization-tree="currentTabState.rootNode || {}" @success="handleSuccess" />
    <IamOrganizationDetailDialog v-model="currentTabState.dialogVisible.detail" :organization-id="currentTabState.currentOrganizationId" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import IamOrganizationEditDialog from '@/modules/iam/organization/IamOrganizationEditDialog.vue'
  import IamOrganizationDetailDialog from '@/modules/iam/organization/IamOrganizationDetailDialog.vue'
  import IamOrganizationUpdateParentDialog from '@/modules/iam/organization/IamOrganizationUpdateParentDialog.vue'
  ...
  interface TabState {
    ...
    // 表格数据
    allData: IamOrganizationExpandTreeResponseVo[]
    displayData: IamOrganizationExpandTreeResponseVo[]
    expandedRowKeys: string[]

    // 树数据
    rootNode: IamOrganizationExpandTreeResponseVo | null

    // 当前选中节点
    currentNode: IamOrganizationExpandTreeResponseVo | null
    ...
    // 搜索工具
    nameFuse: Fuse<IamOrganizationExpandTreeResponseVo> | null
    codeFuse: Fuse<IamOrganizationExpandTreeResponseVo> | null
  }
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      align: 'center',
      fixed: 'right',
      cellRenderer: ({ rowData }: { rowData: IamOrganizationExpandTreeResponseVo }) =>
        h(TableActions, {
          rowData,
  ...
  const fetchTabOptions = async () => {
    try {
      state.tabs = await enumStore.getEnumDataAsync('IamOrganizationTypeEnum')
  ...
  const performSearch = () => {
    ...
    let matchedItems: IamOrganizationExpandTreeResponseVo[] = []
    ...
    // 构建搜索结果树
    const buildResultTree = (nodes: IamOrganizationExpandTreeResponseVo[]): IamOrganizationExpandTreeResponseVo[] => {
      return nodes
  ...
  const handleAdd = (row: IamOrganizationExpandTreeResponseVo | null) => {
    const tabState = currentTabState.value
    tabState.currentNode = row || tabState.rootNode
    tabState.dialogVisible.create = true
  }

  const handleEdit = (row: IamOrganizationExpandTreeResponseVo) => {
    const tabState = currentTabState.value
    tabState.currentOrganizationId = row.id
    tabState.dialogVisible.edit = true
  }

  const handleChangeParent = (row: IamOrganizationExpandTreeResponseVo) => {
  ...
  const handleDelete = async (row: { id: string }) => {
    try {
      await IamOrganizationApi.destroy({ id: row.id })
      handleSuccess()
    } catch (error) {
