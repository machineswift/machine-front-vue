  ...
  >
    ...
    <template #footer>
      ...
      <el-button
        ...
        v-hasPermission="['SYSTEM:SCM:FRONT_CATEGORY:UPDATE_PARENT']"
      >
        确定
      </el-button>
    </template>
...
<script setup lang="ts">
  ...
  const props = defineProps({
    categoryTree: {
      type: Object as PropType<ScmFrontCategoryTreeSimpleResponseVo | null>,
      required: false,
      default: () => ({ children: [] })
    }
  })
  ...
  const handleSubmit = async () => {
    ...
    try {
      state.loading = true
      await ScmFrontCategoryApi.updateParent({
        id: state.form.id,
