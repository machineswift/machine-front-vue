  ...
  <div class="member-page-div">
      ...
      <el-card class="box-card-form" v-show="state.showSearchCard">
        <el-form :model="state.searchForm" ref="searchFormRef" class="search-form" :inline="true" label-width="100px">
          <div class="form-items-group">
            <el-form-item label="会员编码:" prop="code" class="form-item-responsive">
              <el-input v-model="state.searchForm.code" placeholder="请输入会员编码" clearable @keyup.enter="handleSearch" />
            </el-form-item>

            <el-form-item label="邮箱:" prop="email" class="form-item-responsive">
              <el-input v-model="state.searchForm.email" placeholder="请输入邮箱" clearable @keyup.enter="handleSearch" />
            </el-form-item>
    ...
    <el-card class="box-card-data">
      ...
      <el-table :data="state.tableData" border style="margin: 10px 0" v-loading="state.loading" :height="state.dataCardHeight" stripe highlight-current-row>
        ...
        <el-table-column prop="code" label="会员编码" align="center" width="120" fixed></el-table-column>
        ...
        <el-table-column prop="gender" label="性别" align="center" width="80">
          <template #default="{ row }">
            {{ getMemberGenderLabel(row.gender) }}
          </template>
        </el-table-column>
        <el-table-column prop="birthYear" label="出生年份" align="center" width="120">
          <template #default="{ row }">
            {{ row.birthYear || '-' }}
          </template>
        </el-table-column>
    ...
    <!-- 对话框组件 -->
    <CrmMemberAddDialog v-model="state.dialog.add" @success="handleAddSuccess" />
    <CrmMemberDetailDialog v-model="state.dialog.detail" :memberId="state.currentMemberId" />
    <CrmMemberEditDialog v-model="state.dialog.edit" :memberId="state.currentMemberId" @success="handleEditSuccess" />
  </div>
</template>
...
<script setup lang="ts">
  ...
  import type { CrmMemberDetailResponseVo, CrmMemberExpandPageResponse, CrmMemberQueryPageRequestVo } from '@/modules/crm/member/type/CrmMember.type'
  import CrmMemberAddDialog from '@/modules/crm/member/CrmMemberAddDialog.vue'
  import CrmMemberDetailDialog from '@/modules/crm/member/CrmMemberDetailDialog.vue'
  import CrmMemberEditDialog from '@/modules/crm/member/CrmMemberEditDialog.vue'
  ...
  const state = reactive({
    ...
    currentMemberId: '',
    tableData: [] as CrmMemberDetailResponseVo[],
  ...
  // 构建查询参数
  const buildQueryParams = (): CrmMemberQueryPageRequestVo => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res: CrmMemberExpandPageResponse = await CrmMemberApi.pageExpand(params)
  ...
  const showDetailDialog = (row: CrmMemberDetailResponseVo) => {
    state.currentMemberId = row.id
    state.dialog.detail = true
  }

  const showEditDialog = (row: CrmMemberDetailResponseVo) => {
    state.currentMemberId = row.id
    state.dialog.edit = true
  }

  // 删除操作
  const handleDelete = async (row: CrmMemberDetailResponseVo) => {
    try {
      await ElMessageBox.confirm(`确定要删除会员 ${row.name || row.code} 吗？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })

      await CrmMemberApi.destroy({ id: row.id })
      await fetchData()
    } catch (error) {
      if (error !== 'cancel') {
