<template>
  <el-dialog v-model="state.dialogVisible" title="新增素材" :close-on-click-modal="false" :destroy-on-close="true" @closed="handleDialogClosed" width="520px">
    <el-alert type="info" :closable="false" show-icon style="margin-bottom: 12px">仅创建素材记录；如需同时上传文件，请使用页面上方「上传素材」。</el-alert>
    ...
    <template #footer>
      ...
      <el-button type="primary" @click="submitForm" :loading="state.submitting" v-hasPermission="['SYSTEM:BASIC_DATA:MATERIAL:CREATE']">确定</el-button>
    </template>
  </el-dialog>
</template>
