<template>
  ...
  >
        ...
        <el-form ref="formRef" :model="state.formData" :rules="rules" label-width="100px" label-position="right">
          ...
          <el-form-item label="权限类型" prop="resourceType">
            <el-radio-group v-model="state.formData.resourceType" disabled>
              <el-radio v-for="item in permissionResourceTypes" :key="item.code" :value="item.code" border>
                {{ item.message }}
              </el-radio>
            </el-radio-group>
          </el-form-item>
    ...
    <template #footer>
      ...
      <el-button type="primary" @click="handleSubmit" :loading="state.submitting" v-hasPermission="['MANAGE_APP:SYSTEM:ACCESS_CONTROL:PERMISSION:UPDATE']">
        确认
      </el-button>
    </template>
  </el-dialog>
...
<script setup lang="ts">
  ...
  const handleSubmit = async () => {
    try {
      ...
      await BIamPermissionApi.update(updateData)
