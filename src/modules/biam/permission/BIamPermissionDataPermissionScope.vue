          ...
          <div class="scope-selection-area">
            <div class="scope-checkbox-group">
              <el-checkbox-group v-model="item.selectedScopes" @change="handleScopeSelection(index)" :disabled="disabled">
                <el-checkbox v-for="scope in dataPermissionScopeTypes" :key="scope.code" :label="scope.code" :value="scope.code" border size="default">
                  {{ scope.message }}
                </el-checkbox>
              </el-checkbox-group>
            </div>
          ...
          >
            <div class="scope-list-container">
              <draggable v-model="item.scopeList" handle=".drag-handle" item-key="scopeCode" class="scope-list" :disabled="disabled">
                <div v-for="(element, scopeIndex) in item.scopeList" :key="element.scopeCode" class="scope-item">
...
<script setup lang="ts">
  ...
  import { type TabPaneName, type FormInstance, type FormItemRule } from 'element-plus'
  import { useEnumOptions } from '@/shared/composables/useEnumOptions'
  import { DICT_IAM_DATA_PERMISSION_SCOPE_TYPE } from '@/shared/constants/DictionaryEnum.constant'
  ...
  const isPredefinedScope = (scopeCode: string) => {
    return dataPermissionScopeTypes.value.some(scope => scope.code === scopeCode)
  }

  // 验证规则
  const scopeListRules: FormItemRule[] = [
    {
  ...
  const functionNameRules = (index: number): FormItemRule[] => [
  ...
  const functionCodeRules = (index: number): FormItemRule[] => [
  ...
  const scopeNameRules = (tabIndex: number, scopeIndex: number, element: DataPermissionMetaScopeDto): FormItemRule[] => [
    ...
    { min: 2, max: 24, message: '长度在 2 到 24 个字符', trigger: ['blur', 'change'] },
    {
      validator: (_, value, callback) => {
        ...
        if (isDuplicate) {
          ...
          // 检查是否与预定义的范围名称冲突
          const isConflict = dataPermissionScopeTypes.value.some(scope => scope.message === value)
          if (isConflict) {
  ...
  const scopeCodeRules = (tabIndex: number, scopeIndex: number, element: DataPermissionMetaScopeDto): FormItemRule[] => [
    { required: true, message: '请输入范围编码', trigger: ['blur', 'change'] },
    {
      validator: (_, value, callback) => {
        ...
        if (isDuplicate) {
          ...
          // 检查是否与预定义的范围编码冲突
          const isConflict = dataPermissionScopeTypes.value.some(scope => scope.code === value)
          if (isConflict) {
  ...
  const handleScopeSelection = (tabIndex: number) => {
    ...
    ;(currentTab.selectedScopes ?? []).forEach(scopeCode => {
      if (!currentTab.scopeList.some(s => s.scopeCode === scopeCode)) {
        const scope = dataPermissionScopeTypes.value.find(s => s.code === scopeCode)
        if (scope) {
  ...
  watch(
    [() => props.modelValue, dataPermissionScopeTypes],
    async ([modelValue]) => {
      if (modelValue) {
        //状态
