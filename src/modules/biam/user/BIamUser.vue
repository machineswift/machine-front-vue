  ...
  <div ref="pageContainerRef" class="user-page-div">
    <el-splitter>
      ...
      <el-splitter-panel :min="200">
                ...
                <el-table-column label="操作" align="center" width="200" fixed="right">
                  <template #default="{ row }">
                    <div class="table-actions">
                      ...
                      <el-dropdown
                        trigger="click"
                        @command="(command: string | number | object) => handleUserDropdownCommand(command as string, row)"
        ...
        <!-- 对话框组件 -->
        <BIamUserAddDialog v-model="state.dialog.add" @success="handleAddSuccess" />
        <BIamUserDetailDialog v-model="state.dialog.detail" :userId="state.currentUserId" />
        <BIamUserEditDialog v-model="state.dialog.edit" :userId="state.currentUserId" @success="handleEditSuccess" />
        <BIamUserEditPhoneDialog
          v-model="state.dialog.phone"
          :userId="state.currentUserId"
          :current-phone="state.currentUserPhone"
          @success="handlePhoneUpdateSuccess"
        />
        <BIamUserEditPasswordDialog v-model="state.dialog.password" :userId="state.currentUserId" />
        <BIamUserEditPermissionDialog v-model="state.dialog.permission" :userId="state.currentUserId" @success="handlePermissionUpdateSuccess" />
        <BIamRoleQuickSelectDialog v-model="state.roleDialogVisible" @confirm="handleQueryRoleSelect" :multiple="true" :selected-roles="state.selectedRoles" />
...
<script setup lang="ts">
  ...
  import { onMounted, onActivated, reactive, computed, ref, watch, nextTick, onBeforeUnmount } from 'vue'
  import { ElTreeV2 } from 'element-plus'
  import { useRouter } from 'vue-router'
  import { ElMessageBox } from 'element-plus'
  import type { TableInstance, TreeNodeData } from 'element-plus'
  ...
  import { DICT_STATUS, DICT_GENDER, DICT_IAM_ORG_TYPE } from '@/shared/constants/DictionaryEnum.constant'
  import type {
    BIamUserExpandListResponseVo,
    BIamUserExpandPageResponse,
    BIamUserExportRequestVo,
    BIamUserQueryPageRequestVo,
    BIamUserUpdateStatusRequestVo
  } from '@/modules/biam/user/type/BIamUser.type'
  ...
  import BIamUserAddDialog from '@/modules/biam/user/BIamUserAddDialog.vue'
  import BIamUserDetailDialog from '@/modules/biam/user/BIamUserDetailDialog.vue'
  import BIamUserEditDialog from '@/modules/biam/user/BIamUserEditDialog.vue'
  import BIamUserEditPhoneDialog from '@/modules/biam/user/BIamUserEditPhoneDialog.vue'
  import BIamUserEditPasswordDialog from '@/modules/biam/user/BIamUserEditPasswordDialog.vue'
  import BIamUserEditPermissionDialog from '@/modules/biam/user/BIamUserEditPermissionDialog.vue'
  ...
  const state = reactive({
    ...
    currentUserId: '',
    currentUserPhone: '',
    tableData: [] as BIamUserExpandListResponseVo[],
  ...
  const handleSelectionChange = (rows: BIamUserExpandListResponseVo[]) => {
    if (isRestoringSelection.value || pendingRestoreIds.value !== null) return
    const currentPageIds = new Set(state.tableData.map((r: BIamUserExpandListResponseVo) => r.id))
    const selectedIds = new Set(rows.map((r: BIamUserExpandListResponseVo) => r.id))
  ...
  const doRestoreTableSelection = (idsToSelect: Set<string>) => {
    ...
    state.tableData.forEach((row: BIamUserExpandListResponseVo) => {
  ...
  const buildExportRequest = (options: { userIdSet?: string[] }): BIamUserExportRequestVo => {
    const { searchForm } = state
    const base: BIamUserExportRequestVo = {
  ...
  const handleExportCommand = async (command: 'currentList' | 'currentSelected' | 'currentCondition') => {
    let request: BIamUserExportRequestVo
    ...
    try {
      await BIamUserApi.exportUser(request)
  ...
  // 构建查询参数
  const buildQueryParams = (): BIamUserQueryPageRequestVo => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res: BIamUserExpandPageResponse = await BIamUserApi.pageExpand(params)
  ...
  const handleQueryRoleSelect = (roles: BIamRoleSimpleListResponseVo[]) => {
  ...
  // 显示编辑用户对话框
  const showEditDialog = (row: BIamUserExpandListResponseVo) => {
    state.currentUserId = row.id
    state.dialog.edit = true
  }

  // 显示用户详情对话框
  const showDetailDialog = (row: BIamUserExpandListResponseVo) => {
    state.currentUserId = row.id
    state.dialog.detail = true
  }

  // 显示修改手机号对话框
  const showPhoneDialog = (row: BIamUserExpandListResponseVo) => {
    state.currentUserId = row.id
  ...
  // 显示修改权限对话框
  const showPermissionDialog = (row: BIamUserExpandListResponseVo) => {
    state.currentUserId = row.id
    state.dialog.permission = true
  }

  // 显示修改密码对话框
  const showPasswordDialog = (row: BIamUserExpandListResponseVo) => {
    state.currentUserId = row.id
    state.dialog.password = true
  }

  /** 处理用户下拉菜单命令 */
  const handleUserDropdownCommand = (command: string, row: BIamUserExpandListResponseVo) => {
  ...
  const toggleStatus = async (row: BIamUserExpandListResponseVo) => {
    try {
      ...
      const params: BIamUserUpdateStatusRequestVo = {
        id: row.id,
        status: row.status
      }

      await BIamUserApi.updateStatus(params)
    } catch (error) {
  ...
  onMounted(async () => {
    await Promise.all([loadUserStatus(), loadUserGenders(), loadOrganizationTypeOptions()])
