<template>
  <div ref="pageContainerRef" class="role-page">
      ...
      <div v-show="tableHeightReady" style="flex: 1; min-height: 0">
        ...
        >
          ...
          <el-table-column prop="type" label="角色类型" align="center" width="120" show-overflow-tooltip>
            <template #default="{ row }">
              <el-tooltip v-if="row.type" :content="enumStore.getEnumLabel(DICT_IAM_ROLE_TYPE, row.type)" placement="top" :append-to-body="true">
                <span class="text-ellipsis">{{ enumStore.getEnumLabel(DICT_IAM_ROLE_TYPE, row.type) }}</span>
              </el-tooltip>
              <span v-else>-</span>
            </template>
          </el-table-column>
    ...
    <!-- 对话框组件 -->
    <BIamRoleAddDialog v-model="state.dialog.addVisible" @success="handleAddSuccess" />
    <BIamRoleEditDialog v-model="state.dialog.editVisible" :role-id="state.selectedRoleId" @refresh="fetchData" />
    <BIamRoleEditPermissionDialog v-model="state.dialog.editPermissionVisible" :role-id="state.selectedRoleId" />
    <BIamRoleDetailDialog v-model="state.dialog.detailVisible" :role-id="state.selectedRoleId" />
...
<script setup lang="ts">
  ...
  import { reactive, onMounted, onActivated, ref, computed, watch, nextTick, onBeforeUnmount } from 'vue'
  ...
  import BIamRoleAddDialog from '@/modules/biam/role/BIamRoleAddDialog.vue'
  import BIamRoleDetailDialog from '@/modules/biam/role/BIamRoleDetailDialog.vue'
  import BIamRoleEditDialog from '@/modules/biam/role/BIamRoleEditDialog.vue'
  import BIamRoleEditPermissionDialog from '@/modules/biam/role/BIamRoleEditPermissionDialog.vue'
  import { useDictionaryEnumStore } from '@/shared/stores/DictionaryEnum.store'
  import { DICT_STATUS, DICT_IAM_ROLE_TYPE } from '@/shared/constants/DictionaryEnum.constant'
  import type {
    BIamRoleExpandPageResponse,
    BIamRoleQueryPageRequestVo,
    BIamRoleExpandListResponseVo,
    BIamRoleUpdateStatusRequestVo
  } from '@/modules/biam/role/type/BIamRole.type'
  ...
  const state = reactive({
    ...
    roleStatus: [] as IamDictionaryEnumInfoResponse[],
    roleTypes: [] as IamDictionaryEnumInfoResponse[],
    ...
    roleList: [] as BIamRoleExpandListResponseVo[],
  ...
  const buildQueryParams = (): BIamRoleQueryPageRequestVo => {
    return {
  ...
  const fetchData = async (): Promise<void> => {
    try {
      ...
      const response: BIamRoleExpandPageResponse = await BIamRoleApi.pageExpand(params)
  ...
  const showPermissionDialog = (row: BIamRoleExpandListResponseVo) => {
    state.selectedRoleId = row.id
    state.dialog.editPermissionVisible = true
  }

  const onRoleDropdownCommand = (command: string | number | object, row: BIamRoleExpandListResponseVo) => {
    handleRoleDropdownCommand(String(command), row)
  }

  const handleRoleDropdownCommand = (command: string, row: BIamRoleExpandListResponseVo) => {
  ...
  const openEditDialog = (role: BIamRoleExpandListResponseVo): void => {
    state.selectedRoleId = role.id
    state.dialog.editVisible = true
  }

  const openDetailDialog = (role: BIamRoleExpandListResponseVo): void => {
    state.selectedRoleId = role.id
    state.dialog.detailVisible = true
  }

  const toggleRoleStatus = async (role: BIamRoleExpandListResponseVo): Promise<void> => {
    try {
      ...
      const params: BIamRoleUpdateStatusRequestVo = {
        id: role.id,
        status: role.status
      }

      await BIamRoleApi.updateStatus(params)
    } catch (error) {
  ...
  const handleDeleteRole = async (role: BIamRoleExpandListResponseVo): Promise<void> => {
    try {
      ...
      await BIamRoleApi.destroy({ id: role.id })
  ...
  onMounted(async () => {
    const [roleStatus, roleTypes] = await Promise.all([enumStore.getEnumDataAsync(DICT_STATUS), enumStore.getEnumDataAsync(DICT_IAM_ROLE_TYPE)])
