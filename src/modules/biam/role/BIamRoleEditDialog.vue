    ...
    <el-form :model="state.formData" label-width="100px" :rules="state.rules" ref="formRef">
      ...
      <el-form-item label="角色类型" prop="type">
        <el-select v-model="state.formData.type" placeholder="请选择角色类型" disabled>
          <el-option v-for="option in roleTypes" :key="option.code" :label="option.message" :value="option.code" />
        </el-select>
      </el-form-item>
...
<script setup lang="ts">
  ...
  import { ref, watch, reactive, computed, nextTick, type ComponentPublicInstance } from 'vue'
    ...
    nextTick(() => {
      ...
      if (state.organizationDefaultCheckedKeysMap.has(code)) {
        ...
        if (checkedKeys && checkedKeys.length > 0) {
          treeEl.setCheckedKeys(checkedKeys)
        }
      }
    })
  }
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      if (state.formData.dataPermissionRule.scopeCode === 'CUSTOM') {
        ...
        // 处理每个组织类型的选中数据
        for (const orgType of organizationTypeOptions.value) {
  ...
  const loadOrganizationTreeData = async () => {
    if (organizationTypeOptions.value.length === 0) return

    const loadPromises = organizationTypeOptions.value.map(async option => {
  ...
  const fetchData = async () => {
    try {
      state.loading = true
      const res = await BIamRoleApi.detail({ id: props.roleId })
