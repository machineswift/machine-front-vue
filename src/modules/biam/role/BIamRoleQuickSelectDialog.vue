  ...
  >
    <div class="role-selector-container">
          ...
          <div class="form-items-group">
            ...
            <el-form-item label="状态:" class="form-item-responsive" prop="status">
              <el-select v-model="state.searchForm.status" placeholder="选择状态" clearable>
                <el-option v-for="option in roleStatus" :key="option.code" :label="option.message" :value="option.code" />
              </el-select>
            </el-form-item>
            <el-form-item label="角色类型:" class="form-item-responsive">
              <el-select v-model="state.searchForm.type" placeholder="请选择角色类型" clearable>
                <el-option v-for="item in roleTypes" :key="item.code" :label="item.message" :value="item.code" />
              </el-select>
            </el-form-item>
          </div>
          <!-- 操作按钮组 -->
      ...
      <el-table
        ...
        @selection-change="rows => handleSelectionChange(rows as BIamRoleSimpleListResponseVo[])"
      ...
      >
        ...
        <el-table-column prop="name" label="角色名称" align="center" width="160" fixed="left" />
...
<script setup lang="ts">
  ...
  import type { BIamRoleQueryPageRequestVo, BIamRoleSimpleListResponseVo } from '@/modules/biam/role/type/BIamRole.type'
  ...
  const { options: roleStatus, load: loadRoleStatus } = useEnumOptions(DICT_STATUS)
  ...
  const state = reactive({
    ...
    roleList: [] as BIamRoleSimpleListResponseVo[],
    selectedRows: new Map<string, BIamRoleSimpleListResponseVo>(),
  ...
  const isRowDisabled = (row: BIamRoleSimpleListResponseVo) => {
    return props.selectedRoles.some(role => role.id === row.id)
  }

  const handleRadioChange = (row: BIamRoleSimpleListResponseVo) => {
  ...
  const handleRowClick = (row: unknown, column: { type?: string } | null) => {
    ...
    if (props.multiple) {
      roleTableRef.value?.toggleRowSelection(row as BIamRoleSimpleListResponseVo, !state.selectedRows.has((row as BIamRoleSimpleListResponseVo).id))
    } else {
      handleRadioChange(row as BIamRoleSimpleListResponseVo)
    }
  }

  const handleSelectionChange = (rows: BIamRoleSimpleListResponseVo[]) => {
  ...
  const fetchRoleList = async () => {
    try {
      state.loading = true
      const params: BIamRoleQueryPageRequestVo = {
      ...
      const res = await BIamRoleApi.pageSimple(params)
    ...
    async val => {
      ...
      if (props.multiple) {
        ...
        props.selectedRoles.forEach(role => {
          state.selectedRows.set(role.id, role as BIamRoleSimpleListResponseVo)
        })
      }
      await fetchRoleList()
    },
