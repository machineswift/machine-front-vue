                ...
                <el-tab-pane v-for="option in props.organizationTypeOptions" :key="option.code" :label="option.message">
                  <div class="tab-header">
                    <el-input
                      ...
                      @input="(val: unknown) => handleOrganizationQueryChange(meta.functionCode, option.code, String(val))"
                      placeholder="请输入关键字"
                      clearable
                      class="search-input"
                    />
                    ...
                    <el-checkbox
                      ...
                      @change="(val: unknown) => handleOrganizationCascadeSelectionChange(meta.functionCode, option.code, val as boolean)"
                    >
                      父子联动
                    </el-checkbox>
                  </div>
                  <el-tree-v2
                    :ref="(el: Element | ComponentPublicInstance | null) => setOrganizationTreeRef(meta.functionCode, option.code, el)"
...
<script setup lang="ts">
  ...
  import { reactive, ref, computed, nextTick, watch, type ComponentPublicInstance } from 'vue'
  import { type ElTreeV2, type TreeNodeData } from 'element-plus'
  ...
  const props = defineProps({
    ...
    node: { type: Object as () => BIamPermissionTreeExpandResponseVo, required: true },
    organizationTypeOptions: { type: Array as () => IamDictionaryEnumInfoResponse[], required: true },
    organizationTreeOptionsMap: { type: Map<string, BIamOrganizationSimpleTreeResponseVo[]>, required: true },
    dataPermissionRuleMap: {
      type: Map<string, DataPermissionRuleDto[]>,
      required: true,
      validator: (map: Map<string, DataPermissionRuleDto[]>) => {
  ...
  const state = reactive({
    ...
    drawerOrganizationTreeOptionsMap: new Map<string, Map<string, BIamOrganizationSimpleTreeResponseVo[]>>(),
  ...
  const organizationFilterMethod = (query: string, node: TreeNodeData): boolean => {
  ...
  const setOrganizationTreeRef = (functionCode: string, orgType: string, el: Element | ComponentPublicInstance | null) => {
    if (!el || el instanceof Element) return
    organizationTreeRefMap.value.set(`${functionCode}-${orgType}`, el as unknown as InstanceType<typeof ElTreeV2>)
  }
  ...
  const initializeOrganizationData = async (functionCode: string) => {
    await Promise.all(
      props.organizationTypeOptions.map(async option => {
        ...
        const defaultExpandedKeys = organizationTreeData && organizationTreeData.length > 0 ? [organizationTreeData[0].id] : []
      ...
      state.currentMenuPermissionNode.dataPermissionMetaList?.forEach(meta => {
        ...
        if (scopeCode === 'CUSTOM') {
          props.organizationTypeOptions.forEach(option => {
            ...
            if (treeRef) {
              const checkedNodes = treeRef.getCheckedNodes() as BIamOrganizationSimpleTreeResponseVo[]
