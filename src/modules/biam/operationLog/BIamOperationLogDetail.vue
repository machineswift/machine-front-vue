<template>
  ...
  >
    <el-form :model="state.detailData" label-width="120px" v-loading="state.loading">
      ...
      <el-divider content-position="left">业务对象</el-divider>

      <el-form-item label="操作内容" v-if="state.detailData.content">
        <el-input :model-value="state.detailData.content" type="textarea" :rows="2" disabled />
      </el-form-item>

      <el-form-item label="变更Diff" v-if="state.detailData.diff">
        <el-input :model-value="state.detailData.diff" type="textarea" :rows="6" disabled />
      </el-form-item>
      ...
      <el-form-item label="扩展信息" v-if="state.detailData.extendInfo">
        <el-input :model-value="formatExtendInfo(state.detailData.extendInfo)" type="textarea" :rows="4" disabled />
      </el-form-item>
...
<script setup lang="ts">
  ...
  import type { BIamOperationLogDetailResponseVo } from '@/modules/biam/operationLog/type/BIamOperationLog.type'
  ...
  const state = reactive({
    ...
    loading: false,
    detailData: {} as Partial<BIamOperationLogDetailResponseVo>
  })
  ...
  // 扩展信息格式化（对象转 JSON 文本）
  const formatExtendInfo = (extendInfo: unknown) => {
    if (extendInfo == null) return ''
    if (typeof extendInfo === 'string') return extendInfo
    try {
      return JSON.stringify(extendInfo, null, 2)
    } catch {
      return String(extendInfo)
    }
  }
  ...
  const fetchData = async () => {
    ...
    try {
      state.loading = true
      const res = await BIamOperationLogApi.detail({ id: props.logId })
